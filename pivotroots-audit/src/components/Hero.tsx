...
import { ENGINES, MINUTES_LABEL } from "@/lib/site";
...
export function Hero({ initialToken = null, params = {} }: { initialToken?: string | null; params?: HeroParams }) {
  ...
  const start = async (v: FormValues) => {
    ...
    try {
      const r = await createAudit({ url: v.domain, email: v.email, brand_name: v.brand, country, campaign });
  ...
  return (
    <section className="hero" id="audit">
      <div className="wrap">
        ...
        <span className="tag acc">New · AI Visibility Audit</span>
        ...
        {!token && (
          <AuditForm busy={busy} serverError={serverError} onSubmit={start}

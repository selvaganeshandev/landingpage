...
for pa in PromptAnalytics.objects.select_related('prompt').all()[:5]:
    print(f'  ID: {pa.id}, Platform: {pa.platform}, Prompt Status: {pa.prompt.track_status}, Is Published: {pa.is_published}')
...
# Fixed: Use prompt__track_status instead of track_status (PromptAnalytics.track_status is never updated)
print('\nCompleted analytics count:', PromptAnalytics.objects.filter(prompt__track_status='COMP', is_published=True).count())
